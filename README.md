...
> **Nota**: Latencias medidas en nRF52840 @ 64 MHz con optimización `-O2`.

---

### 🎨 **Código PlantUML para diagrama de secuencia**

Puedes usar este código en [PlantUML Online](https://www.plantuml.com/plantuml/uml/) o en tu editor con extensión PlantUML:

```plantuml
@startuml
title Flujo de Eventos: Pulsación de Botón en Beat Hero
...
box "Contexto de Interrupción (ISR)" #FFE4E1
    participant "Hardware\nGPIO" as HW
    participant "HAL ISR\nhal_ext_int" as HAL
    participant "Driver Botones\ndrv_botones" as DRV
    participant "FIFO\nrt_fifo" as FIFO
end box

box "Contexto Main Loop" #E1FFE4
    participant "Despachador\nrt_GE" as GE
    participant "Gestor Eventos\nsvc_GE" as SVC
    participant "Aplicación\nbeat_hero" as APP
end box

Usuario -> HW: Pulsación física
activate HW
HW -> HAL: IRQ (EINT0/GPIOTE)
activate HAL #Red
note right of HAL: ⚡ Contexto crítico\n~10 µs
HAL -> DRV: Callback registrado
activate DRV
DRV -> DRV: Identifica botón\nCaptura timestamp
DRV -> FIFO: rt_FIFO_encolar(ev_BOTON, ID)
activate FIFO
FIFO -> FIFO: hal_SC_entrar()\n[Disable IRQ]
FIFO -> FIFO: Escribe en buffer[idx]
FIFO -> FIFO: hal_SC_salir()\n[Enable IRQ]
deactivate FIFO
DRV --> HAL: return
deactivate DRV
HAL --> HW: RETI
...
GE -> GE: Despierta de WFI
activate GE #Green
note right of GE: 🔄 Bucle principal\n~100 µs
GE -> FIFO: rt_FIFO_extraer(&evento)
activate FIFO
FIFO -> FIFO: hal_SC_entrar()\n[Disable IRQ]
FIFO -> FIFO: Lee buffer[idx]
FIFO -> FIFO: hal_SC_salir()\n[Enable IRQ]
FIFO --> GE: evento extraído
deactivate FIFO

GE -> SVC: Buscar callbacks\npara ev_BOTON
activate SVC
SVC -> SVC: Recorre tabla\nFiltra por evento\nOrdena por prioridad
SVC -> APP: callback_boton_pulsado(ID)
activate APP
APP -> APP: Lee estado FSM
APP -> APP: Compara secuencia
APP -> APP: ¿Acierto o Fallo?
APP -> APP: Actualiza puntuación
APP -> APP: Enciende LEDs
APP --> SVC: return
deactivate APP
SVC --> GE: Callbacks ejecutados
deactivate SVC

GE -> GE: drv_wdt_alimentar()
GE -> GE: Vuelve a WFI (idle)
deactivate GE
...
### 🎓 **Explicación para memoria técnica**

Para tu memoria, incluye esta explicación:

> **Separación de contextos de ejecución**
>
> La arquitectura del proyecto Beat Hero implementa una clara separación entre el **contexto de interrupción** (asíncrono, tiempo crítico) y el **contexto de hilo principal** (síncrono, procesamiento diferido). Esta separación es fundamental para:
>
> 1. **Evitar condiciones de carrera**: El uso de secciones críticas (`hal_SC`) en la FIFO garantiza atomicidad en operaciones compartidas entre ISR y main loop.
>
> 2. **Minimizar latencia de ISR**: Las rutinas de interrupción solo realizan operaciones mínimas (lectura de registros, encolado de eventos) retornando en ~20 µs, permitiendo al sistema responder rápidamente a nuevos eventos.
>
> 3. **Desacoplamiento lógico**: La lógica de negocio del juego (comparación de secuencias, actualización de puntuación) se ejecuta en contexto no crítico, evitando bloqueos en ISRs.
>
> 4. **Eficiencia energética**: El main loop puede entrar en modo `WFI` (Wait For Interrupt) cuando la FIFO está vacía, reduciendo el consumo sin perder capacidad de respuesta.
>
> El diagrama de secuencia adjunto ilustra este flujo, destacando visualmente ambos contextos y las garantías de sincronización entre ellos.
